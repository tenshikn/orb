...
def home():
    if request.method == "POST":
        ...
        db.execute("SELECT entry_heading, entry, date_added FROM entries WHERE user_id=? AND title LIKE ?", (session['user_id'], title, ))
    ...
    # All entries
    db.execute("SELECT entry_heading, entry, date_added FROM entries WHERE user_id=?", (session['user_id'], ))
    entries = [dict(row) for row in db.fetchall()]
    entries = sorted(entries, key=lambda row: row['date_added'], reverse=True)
